...
- 사용자가 `EV` 표시 영역을 선택하면 카메라 미리보기 하단에 노출 보정 EV값 설정 패널을 표시한다.
- 사용자가 노출 보정 EV값 설정 패널 외부 영역을 선택하면 노출 보정 EV값 설정 패널을 닫는다.
- 노출 보정 EV값 설정 패널은 `-2`부터 `+2`까지의 범위에서 값을 설정할 수 있다.
- 노출 보정 EV값 설정 패널은 1/3 EV 단위로 값을 조절한다.
- 노출 보정 EV값 설정 패널은 Slider로 EV값을 조절할 수 있다.
- 노출 보정 EV값 설정 패널은 증가 버튼과 감소 버튼으로 EV값을 1/3 EV씩 조절할 수 있다.
- 사용자가 노출 보정 EV값 설정 패널에서 값을 변경하면 카메라 화면은 변경한 EV값으로 노출 보정을 요청한다.
