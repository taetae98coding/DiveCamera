...
- RAW 지원 상태는 `Checking`, `Supported`, `Unsupported`를 구분한다.
- RAW 지원 상태가 `Checking`이면 RAW 미지원 주의 아이콘을 표시하지 않는다.
...
- RAW 지원 상태가 `Checking`이면 RAW 모드에서도 주의 아이콘이 표시되지 않는 TC를 추가한다.
...
- Android/iOS controller의 초기 상태를 `Checking`으로 둔다.
